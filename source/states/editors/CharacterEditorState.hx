...
class CharacterEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	function addCharacterUI()
	{
		...
		noAntialiasingCheckBox.onClick = function() {
			...
			if(!noAntialiasingCheckBox.checked && prefs.antialiasing) {
	...
	override function update(elapsed:Float)
	{
		...
		if(lastZoom != FlxG.camera.zoom) cameraZoomText.text = 'Zoom: ${FlxG.camera.zoom.toDouble()}x';
	...
	function updatePresence()
		DiscordClient.changePresence("In the Character Editor", "Selected " + _char, healthIcon.getCharacter());
	...
	function predictCharacterIsNotPlayer(name:String)
	...
	function newAnim(anim:String, name:String):AnimArray
		return {
			offsets: [0, 0],
			loop: false,
			fps: 24,
	...
	function saveCharacter() {
		...
		try {
			...
			if (data == null) throw Json.lastError;
			...
			trace('Saving character file of "${character.curCharacter}" failed! '.toCMD(RED_BOLD) + e.toString().toCMD(RED));
			onSaveComplete();
		}
	}
}