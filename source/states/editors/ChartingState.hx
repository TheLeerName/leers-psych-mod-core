...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	[
		...
		['EndlessNotes', 'Changes notes color in Endless'],
		['TooSlowFlashinShit', 'Static Event, SimpleJump and Jumpscare\nValue 1: either 1, 2 or 3\n 1 = Static\n 2 = SimpleJump\n 3 = Jumpscare']
	];
	...
	override function create()
	{
		...
		waveformSprite = new FlxSprite(gridBg.x + (SHOW_EVENT_COLUMN ? GRID_SIZE : 0), 0).makeGraphic(1, 1, 0x00FFFFFF).disableAntialiasing();
		waveformSprite.scrollFactor.x = 0;
		waveformSprite.visible = false;
		add(waveformSprite);

		dummyArrow = new FlxSprite().makeGraphic(1, 1, FlxColor.WHITE).disableAntialiasing();
		...
		eventLockOverlay = new FlxSprite(gridBg.x, 0).makeGraphic(1, 1, FlxColor.BLACK).disableAntialiasing();
		...
		timeLine = new FlxSprite(gridBg.x, 0).makeGraphic(1, 1, FlxColor.WHITE).disableAntialiasing();
		...
		mustHitIndicator = FlxSpriteUtil.drawTriangle(new FlxSprite(0, iconY - 20).makeGraphic(16, 16, FlxColor.TRANSPARENT), 0, 0, 16).disableAntialiasing();
		...
		selectionBox = new FlxSprite().makeGraphic(1, 1, FlxColor.CYAN).disableAntialiasing();
		...
		infoText = new FlxText(15, 15, 230, '', 16).disableAntialiasing();
		...
		outputTxt = new FlxText(25, FlxG.height - 50, FlxG.width - 50, '', 20).disableAntialiasing();
		...
		var tipText:FlxText = new FlxText(FlxG.width - 210, FlxG.height - 30, 200, 'Press F1 for Help', 20).disableAntialiasing();
		...
		fullTipText = new FlxText(0, 0, FlxG.width - 200).disableAntialiasing();
	...
	function onChartLoaded()
	{
		...
		// SONG TAB
		songNameInputText.text = PlayState.SONG.path;
	...
	override function update(elapsed:Float)
	{
		...
		if(autoSaveCap > 0)
		{
			...
			if(autoSaveTime >= autoSaveCap #if debug || FlxG.keys.justPressed.NUMPADMULTIPLY #end)
			{
				...
				if(!Paths.isDirectory('backups')) Paths.createDirectory('backups');
				Paths.saveFile('backups/$chartName.$BACKUP_EXT', dataToSave);
		...
		if(PsychUIInputText.focusOn == null && lastFocus == null)
		{
				...
				FlxG.keys.justPressed.C || FlxG.keys.justPressed.V || FlxG.keys.justPressed.A || FlxG.keys.justPressed.S))
			{
				...
				else if((doCut = FlxG.keys.justPressed.X) || FlxG.keys.justPressed.C) // Cut (Ctrl + X) and Copy (Ctrl + C)
				{
					...
					{
						copiedNotes.clearArray();
						copiedEvents.clearArray();
		...
		if(Conductor.songPosition != lastTime || forceDataUpdate)
		{
			...
			var str:String =  '$curTime / $songLength' +
							  '\n\nSection: ${(curDecBeat / 4).toDouble()}' +
							  '\nBeat: ${curDecBeat.toDouble()}' +
							  '\nStep: ${curDecStep.toDouble()}' +
	...
	function loadMusic(?killAudio:Bool = false)
	{
		...
		var songPath:String = PlayState.SONG.path;
		...
		if(killAudio)
		{
			...
			{
				//trace(key, snd);
				if(key.contains('/songs/$songPath/') && snd != null)
				{
					sndsToKill.push(key);
					snd.close();
				}
	...
	function _cacheSections()
	{
		var time:Float = 0;
		var row:Int = 0;
		cachedSectionRow.clearArray();
		cachedSectionTimes.clearArray();
		cachedSectionCrochets.clearArray();
		cachedSectionBPMs.clearArray();
		...
		if(FlxG.sound.music != null && !reachedLimit) //Created sections to fill blank space
		{
			...
			var beat:Float = Conductor.calculateCrochet(bpm);
			var sectionBeats:Float = Conductor.getSectionBeats(lastSection);
	...
	function loadSection(?sec:Null<Int> = null)
	{
		...
		if(curSec > 0)
		{
			...
			prevGridBg.rows = Math.round(4 * Conductor.getSectionBeatsFromSong(curSec - 1) * curZoom);
		...
		if(curSec < PlayState.SONG.notes.length - 1)
		{
			...
			nextGridBg.rows = Math.round(4 * Conductor.getSectionBeatsFromSong(curSec + 1) * curZoom);
		...
		gridBg.rows = Math.round(4 * Conductor.getSectionBeatsFromSong(curSec) * curZoom);
		hei += gridBg.height;
	...
	function addChartingTab()
	{
		...
		var txt = new FlxText(objX, objY, 280, "Any options here won't actually affect gameplay!").disableAntialiasing();
		...
		tab_group.add(new FlxText(hitsoundPlayerStepper.x, hitsoundPlayerStepper.y - 15, 100, 'Hitsound (Player):').disableAntialiasing());
		tab_group.add(new FlxText(hitsoundOpponentStepper.x, hitsoundOpponentStepper.y - 15, 100, 'Hitsound (Opp.):').disableAntialiasing());
		tab_group.add(new FlxText(metronomeStepper.x, metronomeStepper.y - 15, 100, 'Metronome:').disableAntialiasing());
		tab_group.add(hitsoundPlayerStepper);
		tab_group.add(hitsoundOpponentStepper);
		tab_group.add(metronomeStepper);
		
		tab_group.add(new FlxText(instVolumeStepper.x, instVolumeStepper.y - 15, 100, 'Inst. Volume:').disableAntialiasing());
		tab_group.add(new FlxText(playerVolumeStepper.x, playerVolumeStepper.y - 15, 100, 'Main Vocals:').disableAntialiasing());
		tab_group.add(new FlxText(opponentVolumeStepper.x, opponentVolumeStepper.y - 15, 100, 'Opp. Vocals:').disableAntialiasing());
	...
	function addDataTab()
	{
		...
		noteTextureInputText.unfocus = function()
		{
			...
			if(changed)
			{
				...
				if(Paths.exists(textureLoad) || noteTextureInputText.text.trim() == '')
				{
					for (note in notes)
					{
						if(note == null) continue;
		...
		tab_group.add(new FlxText(gameOverCharDropDown.x, gameOverCharDropDown.y - 15, 120, 'Game Over Character:').disableAntialiasing());
		tab_group.add(new FlxText(gameOverSndInputText.x, gameOverSndInputText.y - 15, 180, 'Game Over Death Sound (sounds/):').disableAntialiasing());
		tab_group.add(new FlxText(gameOverLoopInputText.x, gameOverLoopInputText.y - 15, 180, 'Game Over Loop Music (music/):').disableAntialiasing());
		tab_group.add(new FlxText(gameOverRetryInputText.x, gameOverRetryInputText.y - 15, 180, 'Game Over Retry Music (music/):').disableAntialiasing());
		...
		tab_group.add(new FlxText(noteTextureInputText.x, noteTextureInputText.y - 15, 100, 'Note Texture:').disableAntialiasing());
		tab_group.add(new FlxText(noteSplashesInputText.x, noteSplashesInputText.y - 15, 120, 'Note Splashes Texture:').disableAntialiasing());
	...
	function addEventsTab()
	{
		...
		selectedEventText = new FlxText(150, objY + 30, 150, '').disableAntialiasing();
		...
		objY += 40;
		eventDescriptionText = new FlxText(objX, objY, 280, defaultEvents[0][1]).disableAntialiasing();

		tab_group.add(new FlxText(eventDropDown.x, eventDropDown.y - 15, 80, 'Event:').disableAntialiasing());
		tab_group.add(new FlxText(value1InputText.x, value1InputText.y - 15, 80, 'Value 1:').disableAntialiasing());
		tab_group.add(new FlxText(value2InputText.x, value2InputText.y - 15, 80, 'Value 2:').disableAntialiasing());
	...
	function addNoteTab()
	{
		...
		tab_group.add(new FlxText(susLengthStepper.x, susLengthStepper.y - 15, 80, 'Sustain length:').disableAntialiasing());
		tab_group.add(new FlxText(strumTimeStepper.x, strumTimeStepper.y - 15, 100, 'Note Hit time (ms):').disableAntialiasing());
		tab_group.add(new FlxText(noteTypeDropDown.x, noteTypeDropDown.y - 15, 80, 'Note Type:').disableAntialiasing());
	...
	function addSectionTab()
	{
		...
		function copyNotesOnSection(?secOff:Int = 0, ?showMessage:Bool = true) //Used on "Copy Section" and "Copy Last Section" buttons
		{
			...
			if(showMessage)
			{
				if(notesCopyNum == 0 && eventsCopyNum == 0)
					return showOutput('Nothing to copy!', true);
			...
			for (note in curRenderedNotes.members)
			{
				...
				if(side > -1)
				{
					if(Math.floor(note.songData[1] / GRID_COLUMNS_PER_PLAYER) != side)
						return showOutput('You cannot press this button with notes on more than one side.');
				}
		...
		tab_group.add(new FlxText(beatsPerSecStepper.x, beatsPerSecStepper.y - 15, 100, 'Beats per Section:').disableAntialiasing());
	...
	function addSongTab()
	{
		...
		songNameInputText.onChange = (old:String, cur:String) -> Song.setSongName(PlayState.SONG, cur, PlayState.SONG.songDisplay);
		...
		tab_group.add(new FlxText(songNameInputText.x, songNameInputText.y - 15, 80, 'Song Name:').disableAntialiasing());
		...
		tab_group.add(new FlxText(bpmStepper.x, bpmStepper.y - 15, 50, 'BPM:').disableAntialiasing());
		tab_group.add(new FlxText(scrollSpeedStepper.x, scrollSpeedStepper.y - 15, 80, 'Scroll Speed:').disableAntialiasing());
		tab_group.add(new FlxText(audioOffsetStepper.x, audioOffsetStepper.y - 15, 100, 'Audio Offset (ms):').disableAntialiasing());
		tab_group.add(bpmStepper);
		tab_group.add(scrollSpeedStepper);
		tab_group.add(audioOffsetStepper);

		//dropdowns
		tab_group.add(new FlxText(stageDropDown.x, stageDropDown.y - 15, 80, 'Stage:').disableAntialiasing());
		tab_group.add(new FlxText(playerDropDown.x, playerDropDown.y - 15, 80, 'Player:').disableAntialiasing());
		tab_group.add(new FlxText(opponentDropDown.x, opponentDropDown.y - 15, 80, 'Opponent:').disableAntialiasing());
		tab_group.add(new FlxText(girlfriendDropDown.x, girlfriendDropDown.y - 15, 80, 'Girlfriend:').disableAntialiasing());
	...
	function addFileTab()
	{
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Open Chart...', function()
		{
			...
			fileDialog.open(function()
			{
				try
				{
					...
					var loadedChart:SwagSong = Song.parseJSON(fileDialog.data);
					if(loadedChart == null || !Reflect.hasField(loadedChart, 'song')) //Check if chart is ACTUALLY a chart and valid
						return showOutput('Error: File loaded is not a Psych Engine/FNF 0.2.x.x chart.', true);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Open Autosave...', function()
		{
			...
			if(!Paths.existsAbsolute('backups/'))
				return showOutput('The "backups" folder does not exist.', true);

			var fileList:Array<String> = Paths.readDirectory('backups/').filter((file:String) -> file.endsWith('.$BACKUP_EXT'));
			if(fileList.length < 1)
				return showOutput('No autosave files found.', true);
				...
				function(state:BasePrompt) {
					...
					var btn:PsychUIButton = new PsychUIButton(0, radioGrp.y + radioGrp.height + 20, 'Load', function()
					{
						...
						if(Paths.existsAbsolute(path))
						{
							try
							{
								var loadedChart:SwagSong = Song.parseJSON(Paths.text(path));
								if(loadedChart == null || !Reflect.hasField(loadedChart, '__original_path'))
									return showOutput('Error: File loaded is not a valid Psych Engine autosave.', true);
								...
								var func:Void->Void = function()
								{
									Song.chartPath = Paths.existsAbsolute(originalPath) ? originalPath : null;
									loadChart(loadedChart);
									reloadNotesDropdowns();
									prepareReload();
		...
		if(SHOW_EVENT_COLUMN)
		{
			...
			var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Open Events...', function()
			{
				...
				fileDialog.open(function()
				{
					...
					{
						var filePath:String = fileDialog.path.replace('/', '\\');
						var eventsFile:SwagSong = Song.parseJSON(fileDialog.data);
						if(eventsFile == null || Reflect.hasField(eventsFile, 'scrollSpeed') || eventsFile.events == null)
							return showOutput('Error: File loaded is not a Psych Engine chart/events file.', true);
	
						var loadedEvents:Array<Dynamic> = eventsFile.events;
						if(loadedEvents.length < 1)
							return showOutput('Events file loaded is empty.', true);
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Reload Chart', function()
		{
			...
			{
				if(Song.chartPath == null)
					return showOutput('You must save/load a Chart first to Reload it!', true);
	
				if(Paths.existsAbsolute(Song.chartPath))
				{
					try
					{
						var reloadedChart:SwagSong = Song.parseJSON(Paths.text(Song.chartPath));
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Save (V-Slice)...', function()
		{
			...
			fileDialog.openDirectory('Save V-Slice Chart/Metadata JSONs', function()
			{
				try
				{
					...
					var chartName:String = PlayState.SONG.path + '.json';
					chartName = chartName.substring(chartName.lastIndexOf('\\')+1, chartName.lastIndexOf('.'));
					...
					openSubState(new BasePrompt('Metadata',
						function(state:BasePrompt)
						{
							...
							var artistTxt:FlxText = new FlxText(artistInput.x, artistInput.y - 15, 100, 'Artist/Composer:').disableAntialiasing();
							artistTxt.cameras = state.cameras;
							var charterTxt:FlxText = new FlxText(charterInput.x, charterInput.y - 15, 100, 'Charter:').disableAntialiasing();
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Psych to V-Slice...', function()
		{
			...
			fileDialog.open('song.json', 'Open a Psych Engine Chart JSON', function()
			{
				...
				var loadedChart:SwagSong = Song.parseJSON(fileDialog.data);
				if(loadedChart == null || !Reflect.hasField(loadedChart, 'song')) //Check if chart is ACTUALLY a chart and valid
					return showOutput('Error: File loaded is not a Psych Engine 0.x.x/FNF 0.2.x.x chart.', true);

				var pack:VSlicePackage = VSlice.export(loadedChart);
				if(pack.chart == null || pack.metadata == null)
					return showOutput('Error: Chart loaded is invalid.', true);
				...
				openSubState(new BasePrompt('Metadata',
					function(state:BasePrompt)
					{
							...
							try
							{
								...
								if(diffs != null && diffs.length > 0)
								{
									...
									for (diff in diffs)
									{
										...
										if(Paths.existsAbsolute(chartToFind))
										{
											var diffChart:SwagSong = Song.parseJSON(Paths.text(chartToFind));
											if(diffChart != null)
											{
									...
									if(Paths.existsAbsolute(chartToFind))
									{
										var eventsChart:SwagSong = Song.parseJSON(Paths.text(chartToFind));
										if(eventsChart != null)
										{
						...
						var charterTxt:FlxText = new FlxText(charterInput.x, charterInput.y - 15, 100, 'Charter:').disableAntialiasing();
						charterTxt.cameras = state.cameras;
						var difficultiesTxt:FlxText = new FlxText(difficultiesInput.x, difficultiesInput.y - 15, 100, 'Difficulties:').disableAntialiasing();
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  V-Slice to Psych...', function()
		{
			...
			fileDialog.open('chart.json', 'Open a V-Slice Chart file', function()
			{
				...
				if(chart == null || chart.version == null || chart.notes == null || chart.scrollSpeed == null)
					return showOutput('Error: File loaded is not a valid FNF V-Slice chart.', true);
				...
				fileDialog.open('metadata.json', 'Open a V-Slice Metadata file', function()
				{
					...
					if(metadata == null || metadata.version == null || metadata.playData == null || metadata.songName == null ||
						metadata.playData.difficulties == null || metadata.timeChanges == null || metadata.timeChanges.length < 1)
							return showOutput('Error: File loaded is not a valid FNF V-Slice metadata.', true);
							...
							fileDialog.openDirectory('Save Converted Psych JSONs', function()
							{
								...
								function nextChart()
								{
									while(diffs.length > 0)
									{
										...
										return overwriteCheck(path + chartName, chartName, PsychJsonPrinter.print(chartData, ['sectionNotes', 'events']), nextChart, true);
									}
			...
			fileDialog.open(function()
			{
				...
				try
				{
					...
					if(!fmt.startsWith('psych_v1'))
					{
						...
						Paths.saveFile(fileDialog.path, PsychJsonPrinter.print(loadedChart, ['sectionNotes', 'events']));
		...
		btn.onClick = function()
		{
			...
			openSubState(new BasePrompt(400, 160, 'Autosave Settings',
				function(state:BasePrompt)
				{
					...
					var txt1:FlxText = new FlxText(timeStepper.x, timeStepper.y - 15, 100, 'Time (in minutes):').disableAntialiasing();
					txt1.cameras = state.cameras;
					var txt2:FlxText = new FlxText(maxFileStepper.x, maxFileStepper.y - 15, 100, 'File Limit:').disableAntialiasing();
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Waveform...', function()
		{
			...
			openSubState(new BasePrompt(320, 200, 'Waveform Settings',
				function(state:BasePrompt) {
					...
					var txt1:FlxText = new FlxText(input.x, input.y - 15, 80, 'Color (Hex):').disableAntialiasing();
					txt1.cameras = state.cameras;
					state.add(txt1);
					state.add(input);
				}
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Go to...', function()
		{
			...
			openSubState(new BasePrompt(420, 200, 'Go to Time/Section:',
				function(state:BasePrompt)
				{
					...
					var txt1:FlxText = new FlxText(timeStepper.x, timeStepper.y - 15, 100, 'Time (in seconds):').disableAntialiasing();
					var txt2:FlxText = new FlxText(sectionStepper.x, sectionStepper.y - 15, 100, 'Section:').disableAntialiasing();
					txt1.cameras = state.cameras;
					txt2.cameras = state.cameras;
					state.add(txt1);
					state.add(txt2);
					...
					var timeTxt:FlxText = new FlxText(15, state.bg.y + state.bg.height - 75, 230, '', 16).disableAntialiasing();
	...
	{
		for (secNum => section in PlayState.SONG.notes)
			PlayState.SONG.notes[secNum].sectionNotes.clearArray();
		...
		events.sort(PlayState.sortByTime);
		PlayState.SONG.events.clearArray();
		for (event in events)
	...
	function saveChart(canQuickSave:Bool = true)
	{
		...
		{
			Paths.saveFile(Song.chartPath, chartData);
	...
	function goToPlayState()
	{
		...
		setSongPlaying(false);
		updateChartData();
		StageData.loadStage(PlayState.SONG.stage);
	...
	function loadCharacterFile(char:String):CharacterFile {
		if(char != null) {
			try {
				var path:String = Paths.path('characters/$char.json');
