	...
	**/
	...
	function addSongUI():Void
	{
		...
		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 30, 'Load Events', function()
		{
			...
			if (Paths.fileExists(Paths.jsonPath('$songName/events'))) {
				clearEvents();
		...
		for (dir in Paths.getAllFolders('characters')) {
			for (file in Paths.readDirectory(dir)) {
				...
				if (file.endsWith('.json')) {
					...
					if(toCheck.trim().length > 0 && !toCheck.endsWith('-dead') && !characters.contains(toCheck))
						characters.push(toCheck);
				}
			}
		}
		...
		for (dir in Paths.getAllFolders('stages')) {
			for (file in Paths.readDirectory(dir)) {
				...
				if (file.endsWith('.json')) {
					var toCheck:String = file.substr(0, file.length - 5);
					if(toCheck.trim().length > 0 && !stages.contains(toCheck))
						stages.push(toCheck);
				}
			}
		}
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.justPressed.BACKSPACE) {
				...
				MusicBeatState.switchState(new states.editors.MasterEditorMenu(), true);
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[3] != null && note.noteType != null && note.noteType.length > 0) {
				...
				daText.setFormat("VCR OSD Mono", 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
				daText.xAdd = -32;
				daText.yAdd = 6;
				daText.borderSize = 1;
	...
	function loadJson(song:String):Void
	{
		...
		catch(e)
		{
			...
			if(missingText == null)
			{
				...
				missingText.setFormat("VCR OSD Mono", 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
