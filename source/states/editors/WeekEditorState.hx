...
class WeekEditorState extends MusicBeatState
{
	...
	function reloadBG() {
		...
		if(assetName != null && assetName.length > 0) {
			if (Paths.fileExistsAbsolute(Paths.imagePath('menubackgrounds/menu_$assetName'))) {
	...
	function reloadWeekThing() {
		...
		var isMissing:Bool = true;
		if(assetName != null && assetName.length > 0) {
			if (Paths.fileExistsAbsolute(Paths.imagePath('storymenu/$assetName'))) {
		...
		DiscordClient.changePresence("In the Week Editor", "Selected " + weekFileName);
	}
