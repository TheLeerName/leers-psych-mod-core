...
class StageEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	function addStageTab()
	{
		...
		{
			var path:String = Paths.path('stages/$selected.json');
			if (path != null)
			{
	...
	private function onLoadComplete(_):Void
	{
		...
		if(fullPath != null)
		{
			...
			if((fullPath.startsWith(exePath + 'assets/') #if MODS_ALLOWED || fullPath.startsWith(exePath + 'mods/') #end) && fullPath.contains('/images/'))
			{
				...
				{
					if(_makeNewSprite == 'animatedSprite' && !Paths.exists('images/$imageToLoad.xml') &&
						!Paths.exists('images/$imageToLoad.json') && !Paths.exists('images/$imageToLoad.txt'))
					{
